...
async function addFavorite(story) {
  ...
  await addAndRemoveFavorite("add", story);
}

async function removeFavorite(story) {
  favorites = favorites.filter(fav => fav.storyId !== story.storyId);
  await addAndRemoveFavorite("remove", story);
}

async function addAndRemoveFavorite(todo, story) {
  const method = todo === "add" ? "POST" : "DELETE";
...
async function loadSavedFavorites() {
  ...
  for (let fav of favs) {
    ...
    $(`#${fav.storyId} .favorite`).css("color", "yellow")
  } 
};
...
async function deleteStory(story) {
  try {
    ...
    console.error("you can't delete someone elses story!")
    return null;
  }
}